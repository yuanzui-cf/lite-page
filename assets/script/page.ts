...
export class litePage {
  ...
  setLink(link: Link[]) {
    link.forEach((value) => {
      try {
        let ele: NodeListOf<HTMLDivElement> = <NodeListOf<HTMLDivElement>>(
          document.querySelectorAll(".button#" + value.id)
        );
        ele.forEach((e) => {
      ...
      fetch(link)
        ...
        .then((r) => {
          ...
          item.forEach((e) => {
            ...
            // get content
            title.textContent =
              e.querySelector("title")!.textContent || "No title";
            des.textContent =
              e.querySelector("description")?.textContent ||
              e.querySelector("content")?.textContent ||
              "No description";
        ...
        .catch((err) => {
          ...
          document.querySelector(".post")!.innerHTML =
            `Cannot connect to the blog: ${err}`;
        });
    } else {
